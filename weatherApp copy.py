...
st.image('HOMEPAGE PICTURE.png', width =500)

st.subheader("For Weather related questions, Ask Siri and she Will Give you A Reply")

if st.sidebar.button('Read Usage Instructions'):
...
if st.button("How Can I Be Of Help To You Today?"):
...
# Set the default value to 'Abuja' if no match is found
    if not place:
        place = 'Abuja'
    place_data, temp, press, wind_speed, precip, model_data = get_data(place[0])# ....... Call the get_data function to fetch place data.

    selected_date = st.date_input("Select a date within the next 5 days only") # ------------ Collect the date
    ...
    if 'rainfall' in your_words_in_text.lower().strip():
        ...
        if int(prediction) == 0:
            ...
            st.image("SUNSHINE.png", WIDTH =500)
            st.text("Have a beautiful day today")
        elif int(prediction) == 1:
            st.success('There will be light rain')
            st.image("LITTLE RAIN.png", width = 500)
            st.text("The sky will be a bit cloudy. Its okay to pack an Umbrella")
        elif int(prediction) == 2:
            st.success('There will be moderate level of rain')
            st.image("MODERATE RAIN.png", width = 500)
            st.text("Ensure to pack an Umbrella. Drink warm tea or coffee to stay warm")
        else:
            st.success('There will be heavy rain')
            st.image("HEAVY RAIN.png", width = 500) 
            st.text("This is the best weather to stay with your loved ones. It is advisable to call in sick and stay indoors today")
    ...
    st.warning('Pls note that I will automatically select Abuja in the absence of a location')
    # else:
